  ...
  if (discovery?.path === undefined || !game) {
    ...
    return Promise.resolve()
      ...
      .then((data) => {
        ...
        lines.forEach((plugin) => {
          gameNativePlugins.add(plugin.toLowerCase());
        });
      })
      .catch((err) => {
...
export function revisionText(gameMode: string): string {
  if (gameMode === "skyrimse") {
    return (
      "This plugin was created for the original Skyrim and may be incompatible " +
